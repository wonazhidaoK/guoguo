  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 100%;box-sizing:border-box; padding:0 50px;">
        ...
        <el-form-item label="小区" prop="SmallDistrictArray">
          ...
          <el-select v-model="temp.SmallDistrictArray" class="filter-item" placeholder="小区">
            <el-option v-for="item in smalllist" :key="item.Name" :label="item.Name" :value="item.Id" :disabled="temp.Id!=''&&temp.Id!=null"/>
          </el-select>
        </el-form-item>
        <div class="editor-container">
          <!-- 上传图片POST /api/uploadVote POST /api/uploadStationLetter-->
          <dropzone v-if="dialogFormVisible" id="myVueDropzone" :url="rootUrl+'/api/uploadStationLetter'" @dropzone-fileAdded="addfile" @dropzone-error="imgError" @dropzone-removedFile="dropzoneR" @dropzone-success="dropzoneS" />
        </div>
      </el-form>
...
import { createvLetter, fetchListVote, updateVote, delVote, detailVote, getSmallList } from '@/api/jdbLetter'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        SmallDistrictArray: [{ required: true, message: '小区是必填的', trigger: 'change' }]
      },
      voteDetailCon: '',
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.num < 2) {
            this.createLoading = true
            createvLetter(this.temp).then((response) => {
