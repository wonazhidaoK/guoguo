  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 100%;box-sizing:border-box; padding:0 50px;">
        ...
        <el-form-item label="小区" prop="SmallDistricts">
          <!-- <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox> -->
          <el-checkbox-group v-model="temp.SmallDistricts">
            <el-checkbox v-for="item in smalllist" :label="item.Id" :key="item.Id">{{ item.Name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <div class="editor-container">
          <!-- 上传图片 -->
          <dropzone v-if="dialogFormVisible" id="myVueDropzone" :url="rootUrl+'/api/uploadAnnouncement'" @dropzone-error="imgError" @dropzone-fileAdded="addfile" @dropzone-removedFile="dropzoneR" @dropzone-success="dropzoneS" />
        </div>
      </el-form>
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="公告详情">
      <div>
        ...
        <h5>小区范围</h5>
        <ul>
          <li v-for="(a,d) in noticeDetailCon.SmallDistrict" :key="d">
            {{ a.Name }}
          </li>
        </ul>
      </div>
...
import { createNotice, fetchListNotice, updateLy, delNotice, getSmallList } from '@/api/jdbNotice'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        SmallDistricts: [{ required: true, message: '小区是必填的', trigger: 'change' }]
      },
