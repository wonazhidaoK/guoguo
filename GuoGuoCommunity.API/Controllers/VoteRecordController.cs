...
namespace GuoGuoCommunity.API.Controllers
{
    ...
    public class VoteRecordController : BaseController
    {
        ...
        public async Task<ApiResult<GetAllFeedbackOutput>> GetFeedback([FromUri]GetFeedbackInput input, CancellationToken cancellationToken)
        {
            ...
            var operationList = await _ownerCertificationRecordRepository.GetListForIdArrayIncludeAsync(data.Select(x => x.OwnerCertificationId.ToString()).ToList());
            foreach (var item in data)
            {
                string OperationName = operationList.Where(x => x.Id.ToString() == item.OwnerCertificationId).FirstOrDefault()?.Owner.Name;
